"""
this experiment is designed for examine transportation mode RMSE at each time step
"""
import os
...
def load_markov_result(path, mode):
    ...
    for t in markov_t_mesh_count:
        if t % 2 == 0:
            ...
            for mesh in (markov_t_mesh_count[t].keys() + markov_t_mesh_count[t + 1].keys()):
                if mesh in markov_t_mesh_count[t + 1] and mesh in markov_t_mesh_count[t]:
                    hour_t_mesh_count[t / 2][mesh] = markov_t_mesh_count[t][mesh] + markov_t_mesh_count[t + 1][mesh]
                elif mesh in markov_t_mesh_count[t + 1]:
                    hour_t_mesh_count[t / 2][mesh] = markov_t_mesh_count[t + 1][mesh]
                else:
                    hour_t_mesh_count[t / 2][mesh] = markov_t_mesh_count[t][mesh]
...
def main():
    ...
    sim_path = "/home/ubuntu/Data/PT_Result/exp3/"
    ...
    # markov_t_mesh_count = load_markov_result(markov_path, "stay")

    coef, r2, rmse = mode_rmse(sim_t_mesh_count, truth_t_mesh_count)
