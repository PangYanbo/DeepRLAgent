...
class Graph(object):
    ...
    def __init__(self, nodes=None, edges=None, directed=False, normalization=True):
        ...
        # # 2018/08/16 add function: map edge to unique number, and vice versa -> move to env class
        # self._action_no = dict(zip(self._edges, range(len(self._edges))))
        # self._no_action = {values: key for key, values in self._action_no.items()}
